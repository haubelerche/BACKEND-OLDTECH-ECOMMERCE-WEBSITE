...
public interface SellerDashboardRepository extends JpaRepository<SellerDashboard, Integer> {
    ...
    /**
     * Lấy dữ liệu dashboard cho biểu đồ trend với period type cụ thể
     */
    @Query("SELECT sd FROM SellerDashboard sd WHERE sd.sellerId = :sellerId " +
           "AND sd.reportDate BETWEEN :startDate AND :endDate " +
           "AND sd.periodType = :periodType " +
    ...
    /**
     * Lấy dữ liệu 9 tháng gần nhất cho ARIMA forecasting - Revenue
     */
    @Query("SELECT YEAR(sd.reportDate) as year, MONTH(sd.reportDate) as month, " +
           "sd.reportDate, sd.totalRevenue " +
           "FROM SellerDashboard sd WHERE sd.sellerId = :sellerId " +
           "AND sd.periodType = 'MONTHLY' " +
           "AND sd.isForecast = false " +
           "AND sd.reportDate >= :startDate " +
           "ORDER BY sd.reportDate ASC")
    ...
    /**
     * Lấy dữ liệu 9 tháng gần nhất cho ARIMA forecasting - Orders
     */
    @Query("SELECT YEAR(sd.reportDate) as year, MONTH(sd.reportDate) as month, " +
           "sd.reportDate, sd.totalOrders " +
           "FROM SellerDashboard sd WHERE sd.sellerId = :sellerId " +
           "AND sd.periodType = 'MONTHLY' " +
           "AND sd.isForecast = false " +
           "AND sd.reportDate >= :startDate " +
           "ORDER BY sd.reportDate ASC")
    List<Object[]> getMonthlyOrdersDataForArima(
            @Param("sellerId") Integer sellerId,
            @Param("startDate") LocalDate startDate);

    /**
     * Lấy dữ liệu 9 tháng gần nhất cho ARIMA forecasting - Customers/Visits
     */
    @Query("SELECT YEAR(sd.reportDate) as year, MONTH(sd.reportDate) as month, " +
           "sd.reportDate, sd.totalVisits, sd.uniqueVisitors " +
           "FROM SellerDashboard sd WHERE sd.sellerId = :sellerId " +
           "AND sd.periodType = 'MONTHLY' " +
           "AND sd.isForecast = false " +
           "AND sd.reportDate >= :startDate " +
           "ORDER BY sd.reportDate ASC")
    List<Object[]> getMonthlyCustomersDataForArima(
            @Param("sellerId") Integer sellerId,
            @Param("startDate") LocalDate startDate);

    /**
     * Tính toán conversion rate trung bình trong khoảng thời gian
     */
    @Query("SELECT " +
           "CASE WHEN SUM(sd.totalVisits) > 0 THEN " +
           "CAST(SUM(sd.totalOrders) AS DOUBLE) / CAST(SUM(sd.totalVisits) AS DOUBLE) * 100 " +
           "ELSE 0 END " +
           "FROM SellerDashboard sd " +
           "WHERE sd.sellerId = :sellerId " +
           "AND sd.reportDate BETWEEN :startDate AND :endDate " +
           "AND sd.isForecast = false " +
           "AND sd.totalVisits IS NOT NULL AND sd.totalVisits > 0")
    Double getAverageConversionRateByPeriod(
            @Param("sellerId") Integer sellerId,
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    /**
     * Tính toán returning customer rate trung bình
     */
    @Query("SELECT " +
           "CASE WHEN SUM(sd.totalOrders) > 0 THEN " +
           "CAST(SUM(sd.returningCustomerOrders) AS DOUBLE) / CAST(SUM(sd.totalOrders) AS DOUBLE) * 100 " +
           "ELSE 0 END " +
           "FROM SellerDashboard sd " +
           "WHERE sd.sellerId = :sellerId " +
           "AND sd.reportDate BETWEEN :startDate AND :endDate " +
           "AND sd.isForecast = false " +
           "AND sd.returningCustomerOrders IS NOT NULL")
    Double getAverageReturningCustomerRate(
            @Param("sellerId") Integer sellerId,
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    /**
     * Lấy dữ liệu cho biểu đồ conversion metrics theo thời gian
     */
    @Query("SELECT sd.reportDate, " +
           "CASE WHEN sd.totalVisits > 0 THEN " +
           "CAST(sd.totalOrders AS DOUBLE) / CAST(sd.totalVisits AS DOUBLE) * 100 " +
           "ELSE 0 END as conversionRate, " +
           "CASE WHEN sd.totalOrders > 0 THEN " +
           "CAST(sd.returnOrdersCount AS DOUBLE) / CAST(sd.totalOrders AS DOUBLE) * 100 " +
           "ELSE 0 END as returnRate, " +
           "CASE WHEN sd.totalOrders > 0 THEN " +
           "CAST(sd.returningCustomerOrders AS DOUBLE) / CAST(sd.totalOrders AS DOUBLE) * 100 " +
           "ELSE 0 END as returningCustomerRate " +
           "FROM SellerDashboard sd " +
           "WHERE sd.sellerId = :sellerId " +
           "AND sd.reportDate BETWEEN :startDate AND :endDate " +
           "AND sd.periodType = :periodType " +
           "AND sd.isForecast = false " +
           "ORDER BY sd.reportDate ASC")
    List<Object[]> getConversionMetricsData(
            @Param("sellerId") Integer sellerId,
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate,
            @Param("periodType") String periodType);
    ...
    /**
     * Lấy summary statistics cho quick stats
     */
    @Query("SELECT " +
           "COALESCE(SUM(sd.totalRevenue), 0) as totalRevenue, " +
           "COALESCE(SUM(sd.totalOrders), 0) as totalOrders, " +
           "COALESCE(SUM(sd.totalVisits), 0) as totalVisits, " +
           "COALESCE(AVG(sd.averageOrderValue), 0) as avgOrderValue " +
           "FROM SellerDashboard sd " +
           "WHERE sd.sellerId = :sellerId " +
           "AND sd.reportDate BETWEEN :startDate AND :endDate " +
           "AND sd.isForecast = false")
    ...
    /**
     * Lấy dữ liệu hôm nay cho quick stats
     */
    @Query("SELECT sd FROM SellerDashboard sd " +
           "WHERE sd.sellerId = :sellerId " +
           "AND sd.reportDate = :today " +
           "AND sd.periodType = 'DAILY' " +
           "AND sd.isForecast = false")
    List<SellerDashboard> getTodayData(
            @Param("sellerId") Integer sellerId,
            @Param("today") LocalDate today);

    /**
     * Lấy dữ liệu tháng hiện tại cho quick stats
     */
    @Query("SELECT sd FROM SellerDashboard sd " +
           "WHERE sd.sellerId = :sellerId " +
           "AND YEAR(sd.reportDate) = YEAR(:currentDate) " +
           "AND MONTH(sd.reportDate) = MONTH(:currentDate) " +
           "AND sd.periodType = 'DAILY' " +
           "AND sd.isForecast = false")
    List<SellerDashboard> getCurrentMonthData(
            @Param("sellerId") Integer sellerId,
